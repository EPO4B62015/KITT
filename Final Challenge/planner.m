...
if car.voltage >= 17 % speed compensation for voltage drop
    speed = 158;
    disp('Voltage level: normal');
elseif car.voltage >= 14
    speed = 162;
    disp('Voltage level: meh');
else
    speed = 165;
    disp('Voltage level: drained');
end
...
if distance <= 0.3
    ...
    if car.did_turn == true
        ...
        steer        = 150;
        car.did_turn = false;
        disp('Driving straight a bit to provide data for orientation.');
    else
        ...
        if abs(car.d_theta) <= 10    %straigth
            ...
            if distance <= 1.5;
                ...
                steer   = 150;
                disp('Almost there, careful now');
            else                %distance is greater than 1.5m
                time    = 1 / car.v_factor;    % return a 1s drive
                steer   = 150;  % straight
                disp('Lets drive in a straight line for a bit');
            end
        else                    % turn
            disp('Lets turn');
